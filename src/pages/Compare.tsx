        ...
        <>
          <div className="flex flex-wrap gap-8 mb-8 justify-center items-stretch">
            {compareItems.map((product: Product) => (
              <div key={product.id} className="flex-1 min-w-[250px] min-w-0 min-h-[320px] p-4 sm:p-6 rounded-xl border shadow bg-white dark:bg-gray-800 flex flex-col gap-2 items-center justify-between">
                <div className="flex items-center justify-center mb-4">
                  <img src={product.image} alt={product.title} className="h-24 w-24 object-contain" />
                </div>
                <h2 className="font-bold text-lg text-blue-700 dark:text-blue-400 mb-1">{product.title}</h2>
                <p className="text-sm text-gray-500 dark:text-gray-300 mb-1">Category: {product.category}</p>
                <p className="text-sm text-gray-500 dark:text-gray-300 mb-1">Price: <span className="font-semibold text-blue-600 dark:text-blue-400">${product.price}</span></p>
                <p className="text-sm text-gray-500 dark:text-gray-300 mb-1">Description: {product.description}</p>
